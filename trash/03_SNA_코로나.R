...
## 세종시 국민신문고
r_sql <- c("select * from tb_api_news_covid;")
...
df$contents <- gsub("코로나","김상훈", df$contents)
tmp_list <- list()
for (i in 1:nrow(df)) {
  tmp <- words(df[i,2])  
  tmp_list[[i]] <- tmp  
...
freq$word <- gsub("김상훈","코로나", freq$word)
...
#tmp_noun <- lapply(noun, function(x) str_subset(x, "/NC$"))
#tmp_noun <- lapply(tmp_noun, function(x) str_replace_all(x, "/NC", ""))
#tmp_noun <- lapply(tmp_noun, function(x) str_replace_all(x, "김상훈", "코로나"))
#tmp_noun <- sapply(tmp_noun, paste, collapse = " ")


## extractnoun case
noun <- sapply(df$contents, extractNoun, USE.NAMES = FALSE)
noun <- lapply(noun, function(x) x[nchar(x)>1])
tmp_noun <- lapply(noun, function(x) str_replace_all(x, "김상훈", "코로나"))
tmp_noun <- sapply(tmp_noun, paste, collapse = " ")
