...
query = URLencode(iconv("인천 서구 신도심", to="UTF-8"))
...
## DB에 데이터 넣기
dbClearResult(dbSendQuery(con,enc2utf8("drop table if exists tb_api_news_new;")))
dbClearResult(dbSendQuery(con,enc2utf8("create table tb_api_news_new (
                                              title text,
                                              contents text,
                                              link text,
                                              date varchar(100)
...
dbClearResult(dbSendQuery(con,enc2utf8("COMMENT ON COLUMN tb_api_news_new.title IS '뉴스제목';")))
dbClearResult(dbSendQuery(con,enc2utf8("COMMENT ON COLUMN tb_api_news_new.contents IS '뉴스내용';")))
dbClearResult(dbSendQuery(con,enc2utf8("COMMENT ON COLUMN tb_api_news_new.link IS '링크';")))
dbClearResult(dbSendQuery(con,enc2utf8("COMMENT ON COLUMN tb_api_news_new.link IS '날짜';")))
dbWriteTable(conn = con,
             name = "tb_api_news_new",
             value = tb_api_news_new,
