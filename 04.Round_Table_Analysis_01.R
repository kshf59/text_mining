...
excel_list <- list()
for (i in 1:5) {
  temp <- read_excel("X:/2020년 프로젝트/06.인천광역시_서구/Data/문화재단_라운드테이블/1회차 라운드테이블 자료.xlsx", sheet = i)
...
## 워드 클라우드 
imagedir <- paste("X:/2020년 프로젝트/06.인천광역시_서구/Script/result/라운드테이블1/", substr(Sys.Date(), 1, 4), sep="")
...
SNA_dtm(dtm, keyword = "문화")


## 단어간의 상관성 파악
findAssocs(tdm, "구도심",0.3)
findAssocs(tdm, "아라뱃길",0.3)
findAssocs(tdm, "프로그램",0.3)
findAssocs(tdm, "어르신",0.3)
checkAssocs(dtm = tdm, keyword = "구도심")
checkAssocs(dtm = tdm, keyword = "신도시")
checkAssocs(dtm = tdm, keyword = "프로그램")
checkAssocs(dtm = tdm, keyword = "어르신")



source("네트워크.R", encoding = "UTF-8")
dt4Networkmap1(workGb = '문화', df = df_tot_mind, sparse = 0.9, corr = 0.1, link = 2, cex = 3)
dt4Networkmap1(workGb = '생각', df = df_tot_mind, sparse = 0.90, corr = 0.3, link = 2, cex = 3)
dt4Networkmap1(workGb = '어르신', df = df_tot_mind, sparse = 0.90, corr = 0.3, link = 2, cex = 3)
